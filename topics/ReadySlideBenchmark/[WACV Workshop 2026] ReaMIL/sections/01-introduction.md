...
> 💡 **问题动机（claude 批注）**: 这里直接限制了“首次发现强诊断模型 attention 不可靠”的表述。ReaMIL 的推进不是再画一张 attention 图，而是在同一联合优化模型内做两种 masking 测试：keep 后真类仍受支持，drop 后真类受抑制。它们衡量模型内部 sufficiency/exclusion，不等于病理语义上的因果必要且充分证据。
