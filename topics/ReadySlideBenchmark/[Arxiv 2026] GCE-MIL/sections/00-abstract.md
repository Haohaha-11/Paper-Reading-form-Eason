...
> 💡 **claude 批注｜摘要证据链**: 输入是 WSI patch bag，GCE 输出 slide prediction 与离散 evidence subset。9 backbone × 9 dataset 的 81 配置覆盖预测主结果，也覆盖 Table 4 聚合的 Sufficiency/Necessity keep-only 与 remove 干预；Table 8 的逐-backbone S/N/R 与 Recoverability 仅展开 BRACS、NSCLC、PANDA，稳定性、定位、消融和成本各有更窄范围。5× 速度仅适用于可选 tile 预筛，不应与缓存特征下的标准验证混为一谈。
