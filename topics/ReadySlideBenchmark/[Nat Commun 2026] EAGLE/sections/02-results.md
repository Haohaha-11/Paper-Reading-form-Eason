...
> 💡 **Figure 1 批读**（框架 + benchmark 规模）（claude 批注）：(b) 的工作流对比是核心——EAGLE 三步（CTransPath 全图粗提 → CHIEF 选 25 tile → Virchow2 精提）vs 常规"全 tile 编码 + 逐任务聚合"。9528 WSI × 13 cohort 的规模 + 严格外部验证（训 TCGA、测 CPTAC/DACHS/Kiel/Bern/IEO，无泄漏）让结论可信。注意 EAGLE 总体第一但**形态学任务输给 TITAN、肺癌输给 TITAN**——稀疏采样对"需要全局架构上下文"的任务有短板（Discussion 承认）。
...
> 💡 **Figure 3 消融解读**（对压缩最关键的三个数字）（claude 批注）：
...
> 💡 **Figure 4 批读**（一个反直觉但深刻的发现）（claude 批注）：这节回答"为什么要用 task-agnostic 的 CHIEF 选 tile，而不是端到端学注意力"。**发现：端到端弱监督训练的 ABMIL 注意力趋向近均匀聚合**（Gini 仅 0.087，需 44% tile 才累积 50% 注意力）——尤其在细微的生物标志物任务上，ABMIL 注意力退化成≈mean pooling！而 CHIEF 的预训练显著性先验高度集中（Gini 0.702，8.4% tile 就占 50%）。
> - **深刻含义**：这与本主题 [Spatial-Blindness](../../../ckmil-re-attn-mil/)、[ACMIL](../../../Whole-Slide-Image-Analysis/%5BECCV%202024%5D%20ACMIL/) 呼应——**端到端学的注意力在弱监督+小样本下不可靠**（要么过度集中过拟合，要么退化成均匀）。EAGLE 的对策是**用大规模预训练的 task-agnostic 显著性先验替代逐任务学的注意力**。对 ReadySlide：这支持"用预训练 FM 的显著性（如 CHIEF/importance_chief）做保留，而非依赖下游任务学的注意力"的路线。
...
> 💡 **Selector–Consumer 成本审计（claude 批注）**: 这组分解是五篇里最接近真实部署的报告：selector 前的 CTransPath 粗提占 2.01 s，CHIEF 排序仅 0.36 ms，昂贵 consumer Virchow2 的 top-25 精提占 0.26 s。它说明跨模型分工的收益主要来自“让贵 consumer 少看”，不是 selector 本身更快。ReadySlideBenchmark 应保持同一成本边界，统一计入粗特征、缓存与 I/O。
