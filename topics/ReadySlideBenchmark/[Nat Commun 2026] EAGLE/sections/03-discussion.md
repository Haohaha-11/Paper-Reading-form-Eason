...
> 💡 **机制拆解**（"25 tile" 的原理性辩护 + 边界）（claude 批注）：这是全文最有理论分量的一段，也是对 ReadySlide 最有价值的洞察：
...
> 💡 **局限与定位解读**（claude 批注）：作者难得诚实——**平均 AUROC 0.742 还不足以替代临床流程**，定位为辅助/分诊。且明确"本研究优先 benchmark 预测性能而非混杂分析"，未做 [Confounders](../../../Whole-Slide-Image-Analysis/%5BNat%20Biomed%20Eng%202026%5D%20Confounders-Biomarker-Prediction/) 式的分层去混杂——**这是 EAGLE 的一个 open question：它选的 25 个"信息 tile"是承载因果诊断信号，还是承载 grade/染色 shortcut？** 结合本目录 Confounders 的方法论，这是评估 EAGLE（及任何 retention 方法）该补的一步。CHIEF 偏癌症 slide 预训练 → 非癌任务上选择可能失效，也是边界。
...
> 💡 **角色交叉缺口（claude 批注）**: 这里是明确的 CHIEF selector→Virchow2 consumer 接口，但论文没有交换二者角色，也没有测试 CHIEF ranking 给 UNI2/CONCH/TITAN 等 consumer 的效用。故可安全表述为“固定跨-FM 配对的直接前作”；完整 selector×consumer×budget 矩阵仍未被覆盖。
...
> 💡 **方法论批读**（EAGLE 可复用的三个方法要素）（claude 批注）：
