...
> **问题动机**: 这是全篇论文最重要的实证动机。作者对真实推理日志的分析揭示了一个"资源浪费悖论"：90%+的长上下文请求只需要最后 8K token 即可正确回答，但现有的 LLM 推理系统无一例外地将全部 KV Cache 保留在 GPU 内存中。这一观察同时暗示了两个关键设计需求：(1) 大部分时间内，系统应该是"内存经济型"的；(2) 但不能简单地滑动窗口丢弃历史，因为剩下的 <10% 的任务需要真正的全局上下文合成。这个"硬矛盾"就是 LSA 要解决的核心问题。

> **机制拆解**: 现有方案（DeepSeek-V4 HCA、Qwen3.5 线性注意力）的局限性在于它们是"压缩型"方案——通过压缩注意力层降低记忆增长速度，但由于必须保留部分全注意力层来维持细粒度事实召回，KV Cache 的线性增长本质并未消除。LSA 的创新在于从"压缩"转向"按需预测与获取"：不改变存储的内容，而是改变何时将什么内容加载到 GPU 的策略。
...
> **机制拆解**: LSA 的三层架构设计：(1) HCA 层——全局压缩上下文感知（128:1），不变；(2) CSA 层——从被动保留全量变为主动预测获取；(3) Memory Indexer——独立训练的双编码器，周期性（τ=64）评估隐藏状态，预测未来窗口需要的查询关键 chunks。解耦训练（Decoupled Training）是关键工程创新——整个索引器训练过程中骨干模型从未被加载到 GPU 内存。
...
> **Q&A 批注记录**: 前两个贡献（LSA 范式 + 解耦训练）是方法论创新，第三个是工程验证。值得注意的是，作者将"效率"而非"准确率提升"作为核心贡献，因为 +0.6% 的准确率提升是在极低内存预算下的副产品，而非主要优化目标。这种"少即是多"的结果需要放在特定基准测试特征下理解（详见 Section 3.3）。
...
> **Figure 2 批读**: 这是理解全论文架构最重要的图。黑色线条代表 DeepSeek-V4 的原生 CSA 流程：每个解码步都从完整上下文中执行压缩注意力 + 稀疏注意力。红色线条代表 LSA 的改进：每 τ=64 步，Memory Indexer 从当前隐藏状态预测未来窗口需要哪些压缩 KV entries，然后从 CPU Cold Pool 按需加载到 GPU，后续的 Lightning Indexer 和核心注意力计算只在这些已加载的 entries 加上不可 offload 的滑动窗口上执行。关键差异在于 GPU 内存中驻留的 KV Cache 从"全量"变为"仅查询关键子集"。注意图中"Cold Pool"的概念与 vLLM/SGLang 中的 CPU offloading 理念一脉相承，但核心区别在于：传统 offloading 是被动的（全部存储，按需加载），而 LSA 是主动预测性的（仅加载预测需要的）。
