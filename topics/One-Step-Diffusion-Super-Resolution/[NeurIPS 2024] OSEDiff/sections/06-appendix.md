...
> 💡 **附录地图**: 附录补的是正文证据链的缺口：GAN baseline、主观评价、更多视觉样例、训练伪代码。尤其 Algorithm 1 对复现 OSEDiff 比正文公式更直接。


# A.1 Comparison with GAN-based Methods
> 💡 **小节预览**: A.1 回答“OSEDiff 相对传统 GAN SR 到底强在哪”。预期不是 PSNR/SSIM 全赢，而是感知质量、FID/CLIPIQA/MUSIQ 和细节自然性更强。
...
> 💡 **GAN 对照批读**: 作者承认 GAN 方法 PSNR/SSIM 更好，这说明 OSEDiff 不是保真派最优；它的优势在更自然的毛发、花瓣、建筑和叶片纹理。这里正好对应 perception-distortion trade-off。
...
> 💡 **User Study 设置**: 主观实验要求志愿者同时考虑 perceptual quality 和 content consistency，避免只投给最锐、最花的图。这对 OSEDiff 很重要，因为它要证明生成细节不是无约束幻觉。
...
> 💡 **User Study 结果**: OSEDiff 第二、略低于 SeeSR，但只用一步且比 SeeSR 快十倍以上。这个结果比单纯第一名更有信息：它说明 OSEDiff 是质量/速度 Pareto 点，而不是主观质量绝对上限。


# A.3 More Visual Comparisons
> 💡 **小节预览**: A.3 用更多视觉样例补 Fig. 3 的覆盖面。阅读重点是不同内容类别下是否稳定，而不是挑单个最漂亮 crop。
...
> 💡 **视觉补充批读**: 作者声称在人像、花纹、建筑、动物毛发和文字场景都可与多步 diffusion 相当。注意 conclusion 又承认 small scene texts 有局限，所以文字案例要放大看笔画是否真实。
...
> 💡 **Table 8 导读**: 这张表是 GAN baseline 的完整量化。预期结果是 GAN 在 fidelity 指标较稳，OSEDiff 在 FID/MUSIQ/CLIPIQA 等感知或语义质量上更突出。
...
> 💡 **Table 8 批读**: DIV2K-Val/DRealSR/RealSR 三组都显示 OSEDiff 的感知优势更明显；但 RealSR 上 PSNR/SSIM 不占优，说明 data loss 并未完全压住生成先验带来的偏移。
...
> 💡 **训练细节批读**: CFG=7.5 和 negative prompts 会影响 VSD 方向，负提示词显式排除 cartoon、CG、watermark、jpeg artifacts、over-smooth 等模式。复现时这些字符串不是可忽略细节。
...
> 💡 **Algorithm 1 批读**: 伪代码显示一次训练迭代分三步：generator forward 得到 $\hat z_H/\hat x_H$，用 MSE+LPIPS 更新 data term，用 latent VSD 更新 regularization，并单独训练 $\epsilon_{\phi'}$。
...
> 💡 **Figure 5 批读**: 与 GAN 方法比，重点看 GAN 是否有局部锐化伪影，OSEDiff 是否生成更自然但可能不完全忠实的纹理。别只看整体“清晰度”。
...
> 💡 **Figure 6 批读**: 这张图对应 user study 的样本和票数。样本只有 20 张、志愿者 15 人，能提供主观趋势，但不是大规模用户偏好结论。
...
> 💡 **Figure 7 批读**: 这里似乎是 MinerU 抽取导致 caption/编号混在一起；阅读时按内容理解为更多主观投票或视觉补充，不要把重复 Figure 7 当新实验。
...
> 💡 **抽取噪声提示**: 这句重复 caption 是 PDF 抽取噪声，不是论文新的技术内容。保留原文即可，批读时不应据此推导额外结论。
...
> 💡 **更多视觉样例**: 多图比较要检查跨类别稳定性：人脸身份、建筑直线、动物毛发、花纹重复模式和文字笔画。OSEDiff 的风险点仍是“合理但不忠实”的细节。
...
| 指标 | 数值 |
|------|------|
| 本节作用 | 补充 GAN 对照、主观实验、更多视觉样例和训练算法 |
| 主线 | 把 LQ latent 直接作为扩散起点，用 latent-space VSD 将 Stable Diffusion 先验压缩到单步 Real-ISR。 |
| 后续关注 | CFG/negative prompt、VSD timestep、user-study 样本量、GAN fidelity trade-off |

### 核心洞察
1. Appendix 强化了 OSEDiff 的定位：不是 fidelity 最优，而是在接近多步 SD 主观质量的同时大幅降延迟。
2. Algorithm 1 给出复现关键：data loss、latent VSD、fine-tuned regularizer、CFG=7.5、negative prompts 和 timestep 采样。
3. 附录也暴露剩余风险：用户研究规模小，视觉样例仍可能选择性展示，小文字和严格结构需要额外评估。

### 可追问点
- 为什么从 LQ latent 而不是纯噪声开始？
- VSD 在这里起什么作用？
- CFG 和 negative prompt 会怎样影响 VSD 正则方向？
- 用户研究是否足以证明内容一致性？
