...
## 📌 预览
本节是核心方法，按数据流读：LQ image + one-step diffusion SR teacher → DIA / Expansion-Corrosion / VAE compression / prompt-time removal → 轻量化 one-step SR student 与实时 SR image。
---
...
> 💡 **方法总览**: TinySR 的压缩对象有三层：denoising transformer 深度、VAE 编解码器、time/prompt/adaLN 条件分支。这样读就不会误以为论文只是在做普通 layer pruning；它真正的目标是把完整推理图变短、变浅、变窄。
...
> 💡 **公式批读**: 这里的 mask 是结构开关：$m_i=1$ 保留第 $i$ 层，$m_i=0$ 跳过该层。深度剪枝之所以接近线性加速，是因为被关掉的 transformer block 在推理时不再执行，而不是只减少少量通道。
...
> 💡 **剪枝判断**: 论文反对只看“当前层重要性”，因为被剪后的 student 还会再蒸馏恢复。对 TinySR 来说，更关键的问题是“剩下的层能不能学回 teacher 行为”，这就是 recoverability。
...
> 💡 **Recoverability**: 这里借 TinyFusion 的思想，但任务从文生图变成 Real-ISR。SR 的输出更受输入结构约束，所以 mask 的好坏不能只看生成自由度，还要看边缘、纹理和退化去除是否能被蒸馏回来。
...
> 💡 **公式批读**: 双层优化可以理解为：内层更新 student 权重让当前 mask 下的模型尽量恢复；外层选择让恢复损失最低的 mask。这样把“剪哪个层”和“剪完能否补救”绑在一起。
...
> 💡 **可微采样**: 二值 mask 本身不能直接反传，所以作者用 Gumbel-Softmax 学一个“选择概率”。读这段要注意：概率高不等于层天然重要，而是表示该剪枝方案在训练中更可恢复。
...
> 💡 **公式批读**: $\tau$ 控制采样从软到硬的程度。训练早期保留探索，后期更接近确定 mask；这和最终部署时固定 student 结构相衔接。
...
> 💡 **公式批读**: 分块独立假设把全局 mask 拆成 block 内局部概率，复杂度下降很明显；代价是每个 block 的保留层数被固定，跨 block 的冗余无法自由迁移。
...
> 💡 **搜索空间**: 这是 DIA 的动机：固定每个 block 剪同样数量，虽然省搜索，但可能把真实最优结构排除掉。Real-ISR 的不同层可能承担不同退化去除和纹理生成角色，均匀剪枝未必合理。
...
> 💡 **Figure 批读**: Fig. 4 是 TinySR 的剪枝核心图：$p(m)$ 先在 block 内学候选 mask，$q(t)$ 再允许相邻 block 之间转移“保留层预算”。Expansion-Corrosion 最后用边际概率决定加回谁、腐蚀谁。
...
> 💡 **公式批读**: 这个组合数证明“block 内独立”只覆盖很小一部分可行 mask。它不是单纯数学铺垫，而是在说明为什么要让 block 之间动态交换保留层。
...
> 💡 **DIA 批读**: Dynamic Inter-block Activation 的关键不是“激活函数”，而是 mask transformation：从一个 block 少留 $k$ 层，同时在另一个 block 多留 $k$ 层，总保留层数不变。这样速度预算固定，但结构分布更灵活。
...
> 💡 **公式批读**: 总概率由原始 block mask 概率和 transformation 概率共同决定。直观上，$p(m)$ 负责“局部怎么剪”，$q(t)$ 负责“层预算往哪个 block 挪”。


where $r$ is the probability of reaching $m$ through $t$ , and is related to expansion-corrosion strategies.
> 💡 **连接点**: $r$ 把 DIA 和 Expansion-Corrosion 接起来：不是随机换层，而是沿着训练中更可能恢复的方向调整 mask。
...
> 💡 **Expansion-Corrosion**: Expansion 是从高边际概率层里加回若干层，Corrosion 是从低边际概率层里拿掉若干层。它把 DIA 的“预算迁移”落到具体 layer 选择上。
...
> 💡 **公式批读**: $\pi_i$ 是 layer 被保留的边际概率，可视为训练后得到的“保留倾向”。最终不是只挑单个最高概率 mask，而是用这些边际概率构造更稳的 expansion/corrosion 操作。
...
> 💡 **实现批读**: 原本 union/intersection 是离散集合操作，无法顺畅反传；作者用连续形式近似，保证 mask 学习还能被 loss 驱动。这是剪枝算法能训练起来的工程细节。
...
> 💡 **公式批读**: 这里用逐元素乘法模拟 corrosion，用加法再扣重叠项模拟 expansion。读法上不用纠结符号细节，抓住“保留可微、保持预算、允许跨 block 调整”即可。
...
> 💡 **关键约束**: Expansion-Corrosion 并不是无限加层找质量，而是在固定激活层数下重排结构。因此它服务的是同等 latency 预算下的质量恢复。
...
> 💡 **最终决策**: 最终 mask 由两步决定：先看 $q(t)$ 选跨 block 转移方向，再看 $\pi$ 选具体层。这个设计比单纯取最大 $p(m)$ 更能利用训练中积累的边际信息。
...
> 💡 **VAE 压缩**: 这是 TinySR 的系统视角：latent diffusion 推理不只有 denoiser，VAE encoder/decoder 也会吃 latency。作者把通道上限剪到 64、移除 attention，并只在 encoder 用 SepConv，因为 decoder 直接影响最终纹理和感知质量。
...
> 💡 **条件模块移除**: TSD-SR 在 SR 中使用固定/默认 prompt，且 TinySR 只做单步推理，因此文本条件和 time embedding 的边际贡献很低。删除这些模块省掉的不只是输入 token，还有 cross-attention、embedding 和相关调制计算。
...
> 💡 **缓存逻辑**: 如果 adaLN 的 shift/scale 训练后几乎不随输入变化，那推理时每张图重复生成它们就是冗余。pre-cache 把动态计算变成常量加载，是很典型的部署优化。
...
> 💡 **速度来源**: 5.68x 不是单一 trick 的结果，而是深度剪枝、VAE 轻量化、条件模块删除和缓存叠加。读消融时要看每一项是否都有独立收益，以及完整模型是否只是把质量损失转移到了不可见案例上。


# 3.3. Training Scheme

VAE Training. We train the VAE encoder $\mathcal { E } _ { t i n y }$ by aligning latent space features using MSE loss:
> 💡 **VAE 训练**: encoder 先对齐原始 VAE latent，让 denoiser 看到的空间不要大幅漂移；decoder 再用感知/GAN loss 补最终图像质感。
...
> 💡 **公式批读**: 这项 MSE 是 tiny encoder 对 pretrained encoder 的 latent 对齐。它保护的是后续 denoising backbone 的输入分布，避免 VAE 压缩后把整条 diffusion SR 管线带偏。
...
> 💡 **Decoder 风险**: decoder 负责把 latent 还原成 HR 图像，压得过狠会直接表现为纹理糊、色彩偏或细节丢失；因此作者没有把 SepConv 也强行放到 decoder。
...
> 💡 **公式批读**: LPIPS 管感知相似，GAN loss 推真实纹理分布；二者用于弥补轻量 decoder 的表达能力损失。这里没有只用 pixel loss，是因为 Real-ISR 不能只追逐平均化重建。
...
> 💡 **公式批读**: 剪枝决策训练同时看 task loss 和 teacher-student distillation。$\epsilon_{stu}$ 对齐 $\epsilon_{tea}$，说明 student 学的是 teacher 的单步去噪/恢复行为，而不是从 GT 重新学一个 SR 模型。
...
> 💡 **训练批读**: LoRA 主要用于剪枝决策和恢复训练阶段降低适配成本；它不是本文的主要贡献，真正要看的是剪枝后的 student 能否通过 teacher distillation 把质量补回来。
...
> 💡 **两阶段恢复**: 第一阶段在 latent space 快速拉近 student/teacher 表示，第二阶段回到 pixel space 加 LPIPS/GAN 修视觉质量。这个顺序对应“先学 teacher 行为，再补图像观感”。
...
> 💡 **公式批读**: latent distillation 用 $z_{stu}$ 对齐 $z_{tea}$，优点是训练更省显存、batch 可以更大；风险是 latent 指标好不等于最终像素纹理一定自然，所以后面还要 stage 2。
...
> 💡 **Figure 批读**: Fig. 5 要按“模块累加”读：每加一个压缩策略，latency/MACs/params 下降多少，MANIQA 是否还能守住。TinySR 的 claim 成立依赖这条效率-质量曲线，而不只是最终速度数字。
...
> 💡 **Figure 批读**: Fig. 6 解释为什么 VAE 值得动手术：通道剪枝影响 down/up/mid block 的计算，attention 在 64-channel VAE 中仍是大头。这个证据支撑“只剪 denoiser 不够”。
...
> 💡 **Figure 批读**: Fig. 7 是移除 prompt 的直观证据：默认 prompt 被剪到 90% 后画质只小幅下降，说明文本条件在该 SR teacher 里主要是历史包袱，不是关键输入。
...
> 💡 **公式批读**: Stage 2 把 latent distillation、LPIPS 和 GAN loss 放在一起，目标是在不偏离 teacher 的前提下补回像素域感知质量。权重设置体现了 fidelity 与 realism 的折中。
...
> 💡 **复现提示**: 方法能不能复现，关键不只在 mask 算法，还在三段训练顺序、LoRA rank、batch size、VAE 预缓存和 teacher latent 的生成方式。
...
| 核心模块 | DIA / Expansion-Corrosion / VAE compression / prompt-time removal |
| 输出 | 轻量化 one-step SR student 与实时 SR image |

### 核心洞察
1. DIA 解决的是固定 block 剪枝搜索空间太窄的问题，通过 $q(t)$ 在相邻 block 之间转移保留层预算。
2. Expansion-Corrosion 把 learned probability 转成具体 mask：高边际概率层被扩张保留，低边际概率层被腐蚀删除。
3. TinySR 是系统压缩：denoiser depth、VAE、time/prompt、adaLN cache 都参与速度收益，任何单点优化都解释不了 5.68x。

### 可追问点
- TinySR 的核心不是新的生成先验吗？
- 为什么要同时压缩 VAE？
- prompt/time 被移除后，TinySR 还能不能迁移到需要文本控制或多步采样的 SR 框架？
