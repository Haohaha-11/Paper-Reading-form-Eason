...
| 项目 | 内容 |
|------|------|
| **标题** | Vision-aligned Latent Reasoing for Multi-modal Large Language Model |
...
*Figure 2: Reasoing length-wise analysis. We investigate the effect of reasoing length on model performance across different MLLMs. We report hallucination rate on MMhalu benchmark and accuracy (%) on MathVista, MathVision, and MMVP benchmark. For MMhalu, lower is better. We observe that VaLR is the only method that exhibits consistent performance improvements as reasoing length increases, while remaining robust on long-horizon tasks.*
...
- **核心方案**: VaLR = Latent Reasoing（潜空间交替推理）+ REPA（表征对齐）+ 两阶段课程学习。
