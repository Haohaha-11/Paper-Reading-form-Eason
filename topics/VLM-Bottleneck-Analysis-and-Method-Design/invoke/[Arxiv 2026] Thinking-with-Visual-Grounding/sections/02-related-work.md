...
Related Work 按时间线梳理了 visually grounded thinking 的演进脉络：从早期的 "region-of-interest selection"（选择关键区域）到 "tightly coupled grounding + reasoing"（grounding 与推理链深度绑定），再到 "grounding as active behavior via RL"（通过强化学习训练动态 grounding 行为）。本文在第三条线上进一步加入 explicit grounding reward。
...
>
> - **Visual CoT**: 在推理链中插入中间 bounding box 来高亮关键区域。本质是 "show where to look"，但 box 和 reasoing text 的关系仍然松散
> - **UV-CoT**: 通过偏好学习 (preference optimization) 减少对人工 box 标注的依赖。但 region selection 仍与推理步骤相对独立

Later work more tightly couples grounding with the reasoing trace. GCoT (Wu et al., 2025), Xia et al. (2025), and Argus (Man et al., 2025) generate grounding coordinates as step-level visual evidence, aiming to make the reasoing more faithful to the image and easier to check.

> 💡 **第二阶段: Grounding + Reasoing 深度耦合**
>
> - **GCoT** (Grounded Chain-of-Thought): 将 grounding 坐标直接作为推理步骤的 visual evidence，目标是让推理 "faithful to the image"
> - **Argus**: Vision-centric reasoing with grounded CoT，强调 grounding 使推理 "easier to check"
...
> - **ViGoRL**: Grounded reinforcement learning for visual reasoing -- 将 RL 用于训练 grounding 行为
> - **VGR**: Visual Grounded Reasoing -- 利用预测的 region 在推理时做 visual replay
...
>
> ```
> Region Selection  →  Coupled Grounding+Reasoing  →  Active RL Grounding
...
- **演进脉络**: Region selection → Coupled grounding+reasoing → Active RL grounding → +Explicit grounding reward
