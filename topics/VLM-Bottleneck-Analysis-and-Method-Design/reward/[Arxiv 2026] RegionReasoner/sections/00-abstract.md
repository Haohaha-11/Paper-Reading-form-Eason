...
| 项目 | 内容 |
|------|------|
| **标题** | RegionReasoer: Region-Grounded Multi-Round Visual Reasoing |
...
> 💡 **Figure 1 批读**: 这张图是理解 RegionReasoer 最关键的一页。展示了一个三轮对话的例子：
>
> **每轮输出的结构化轨迹**:
> - `<scene>`: 全局场景描述
> - `<focus>`: 参考区域的局部描述（包含序列化坐标 bbox=[x1,y1,x2,y2]）
> - `<think>`: 推理过程，**必须显式引用参考框和所需的空间关系**（这是与 VisionReasoer 的核心区别之一）
...
> 💡 **问题动机**: 现有 VLM 推理系统依赖单步或纯文本推理，无法在多视觉上下文中迭代精炼理解。核心挑战在于：(1) 跨轮定位误差如何避免累积？(2) 推理的语义一致性如何随着对话上下文增长而保持？RegionReasoer 通过结构化推理轨迹 (4-tag) + 两条互补奖励信号来解决。
...
- **核心方案**: RegionReasoer = Structured 4-tag Trajectory + Reference Citation Reward + Global--Local Consistency Reward
