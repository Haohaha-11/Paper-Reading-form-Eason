...
```
$o_{t}$ ~ π_θ( · | I, $q_{t}$, $B_{t}^{ref}$, M_{t-1} )
```
...
> 💡 **问题形式化解读**:
> - **状态**: (I, $q_{t}$, $B_{t}^{ref}$, M_{t-1}) —— 图像 + 当前查询 + 参考框集合 + 对话记忆
> - **关键变量 $B_{t}^{ref}$**: 从早期轮次传播或外部提供的参考框，是跨轮定位的核心接口
> - **记忆更新**: $M_{t}$ = M_{t-1} ∪ {($q_{t}$, $o_{t}$)} —— 逐步累积对话历史
...
> 💡 **任务定义**:
> - **检测**: $o_{t}$ = 2D bbox, 指标 = per-turn AP50 + avg across turns
> - **分割**: $o_{t}$ = sparse point or mask, 指标 = per-turn gIoU + avg over images → avg over turns
> - **跨轮参考**: 后期轮次的 $B_{t}^{ref}$ 来自前轮预测的 o_{t-1}, o_{t-2}, ...
...
- **问题定义**: 多轮区域定位推理 = policy π_θ 在每轮根据 (I, $q_{t}$, $B_{t}^{ref}$, M_{t-1}) 产生 $o_{t}$
