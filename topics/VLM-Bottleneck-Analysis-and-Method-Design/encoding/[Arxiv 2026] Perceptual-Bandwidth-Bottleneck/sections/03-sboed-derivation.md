...
```
p($z_{t}$ | ℓ, d*_t) = {
  φ(d*_t) · $E_{y}$[p($z_{t}$ | y, d*_t)] + (1-φ(d*_t)) · p_0($z_{t}$ | d*_t),   if ℓ ∈ d*_t
  p_0($z_{t}$ | d*_t),                                                      if ℓ ∉ d*_t
}
```
