...
沿**单调递增分辨率轨迹** R = {r_1, r_2, ..., r_k} 生成响应 {y_r1, y_r2, ..., y_rk}：
...
- 如果响应**错误** → 标签 Y_label = 1 (**Need-Refine**)，触发 RoI 分支
- 如果响应**正确** → 标签 Y_label = 0 (**No-Refine**)，跳过冗余处理
...
- Prefill 阶段：拼接的视觉+文本 token 通过前 B 层冻结层 → **H_context^B**
- 这些表示通过 R 层可调门控层 → **H_gate^(B+R)**
...
**关键设计**: 隔离用户 query 最后一个 token 的隐藏状态 **H_gate^(B+R)[-1]**：
