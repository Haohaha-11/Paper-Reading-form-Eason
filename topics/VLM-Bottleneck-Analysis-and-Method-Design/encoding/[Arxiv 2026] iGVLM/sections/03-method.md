...
$c_{t}$ = \mathscr{F}_T($T_{{\leq 77}}$), \quad \hat{c}_t = \mathscr{H}_t(\mathrm{Norm}($c_{t}$)), 
...
\gt  💡 **为什么选择 AdaLN 而不是 Cross-Attention？**:
\gt  - **AdaLN**: 通过 scale & shift 参数对特征做仿射变换，不改变 token 序列长度，不引入额外的注意力计算。是一种**轻量级的、逐层的**条件化方式。
\gt  - **Cross-Attention**: 需要让视觉 token 去 attend 文本 token，引入 O(N²) 的额外计算。
\gt  - **Table 5 验证**: iGVLM-Cross (cross-attention 版本) 性能低于原始 iGVLM，且推理开销更大。
\gt  - AdaLN 的关键优势：**hierarchical modulation**——每一层都有独立的 scale/shift 参数，使得浅层调制低级纹理，深层调制语义概念。
...
\gt  💡 **冻结参数 + AdaLN 的可训参数**: CLIP ViT 的原始参数完全冻结，只有 (1) CLIP text encoder 到 vision space 的线性投影 (2) 每层的 AdaLN scale/shift 参数是可训练的。参数量增加极少（Table 4：13.35B → 13.78B），只有约 430M 新增参数，且主要是 AdaLN 的仿射参数。
...
$y_{I}$ = \mathcal{Z}(\mathrm{Norm}($y_{{ct}}$)) + $y_{0}$, 
...
\gt  💡 **Zero-FFN 的关键设计意图 — 渐进式安全初始化**:
\gt  - 公式 (3) 中的 $\mathcal{Z}$ 被零初始化。这意味着在训练开始时，$y_I = 0 + y_0 = y_0$，模型的输出与原始 LLaVA-1.5 **完全一致**。
\gt  - 训练过程中，$\mathcal{Z}$ 的权值逐渐偏离零，指令调制的特征逐步"混入"静态特征。
\gt  - 这种初始化策略保证了：(1) 不破坏预训练先验 (2) 训练稳定 (3) 如果指令调制无效，模型自然退化为 baseline。这是一种**优雅的防御性设计**。

\gt  💡 **两阶段训练策略**:
\gt  - **Stage 1 (Alignment Pretraining)**: 只训练 AdaLN 参数 + 投影层，视觉 backbone 和 LLM 均冻结。学习率 6e-4（比 LLaVA-1.5 的 1e-3 更低）。
\gt  - **Stage 2 (Instruction Tuning)**: 所有组件联合优化。学习率 2e-5。
\gt  - 这种渐进式训练确保了 AdaLN 调制能力先收敛，再与 LLM 协同精调。
...
\gt  | 原则 | 含义 | 为什么重要 |
\gt  |------|------|-----------|
\gt  | Answer Reversal | 用同一问题的选项重排版本来测试鲁棒性（如 Q1: A:Red B:Blue / Q2: A:Blue B:Red） | 排除模型对选项位置的 bias |
\gt  | Multi-Perspective Semantic Diversity | 四问必须从不同语义角度提问（CP/FP/IR/LR/ST/MA） | 强制模型在不同维度上切换视觉关注 |
\gt  | Balanced Answer Distribution | 正确答案在 A/B/C/D 间均匀分布 | 排除猜测策略 |

\gt  💡 **Hierarchical Scoring 的设计智慧**:
\gt  - n=1: 模型答对至少 1/4 即得分 → 测"基本能力"
\gt  - n=2: 答对至少 2/4 → 测"中等一致性"
\gt  - n=3: 答对至少 3/4 → 测"高一致性"
\gt  - n=4: 全部答对 → 测"完美一致性"
\gt  - 随机猜测：n=1 期望约 68%，n=4 期望约 0.4%。所以 n=4 是最严格的指标，区分度最大。
\gt  - 从 Table 2 可以看出，随着 n 增大，所有模型得分都在下降，但 **iGVLM 的下降速度更慢**（尤其是 n=3 和 n=4），这正是 instruction-conditioned perception 在起作用。
