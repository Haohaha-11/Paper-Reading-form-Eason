...
> 💡 **机制拆解 — 两个关键词**:
> - **"Loop Transformer Latent Reasoing"**: 不是 Coconut/Heima 那种"训练一个行为让模型模仿隐式推理"，而是通过 loop transformer 的**结构性递归**天然支持隐空间迭代 refined thinking。
...
### Table 1 (Latent Space Reasoing Approaches Comparison)
