...
> 💡 **公式批读 — Confidence Gain**: Gt = log pi($Y_{gt}$ | I, q, T<=t) - log pi($Y_{gt}$ | I, q, T<t)。衡量加入第 t 步推理后，模型对正确答案置信度的变化。正 Gt 表示该推理步骤"有用"，增强了模型得出正确答案的信心。这是后续 Confidence-Guided Reward 设计的核心预热。
