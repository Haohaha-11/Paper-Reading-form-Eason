...
> **FLOPs 减少的原理**:
>
> 标准 self-attention: O(($N_{t}$ + $N_{v}$)^2 * d)
> 掩码后的 attention: O($N_{t}$ * ($N_{t}$ + $N_{v}$) + M * ($N_{v}$/M)^2) * d = O($N_{t}^{2}$ + $N_{t}$*$N_{v}$ + $N_{v}^{2}$/M) * d
>
> M=10.4 张图时，$N_{v}^{2}$ -> $N_{v}^{2}$/M 意味着视觉 token 间的 attention 计算量减少约 10x。加上 MLP 开销后总体减少约 81% (0.5B model)。
